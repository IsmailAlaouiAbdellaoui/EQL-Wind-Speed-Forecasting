...
class SymbolicNet:
    ...
    def apply_masks(self,masks):
        for i in range(self.depth):
            self.symbolic_layers[i].W = tf.multiply(self.symbolic_layers[i].W, masks[i])
        self.output_weight = tf.multiply(self.output_weight,masks[self.depth])
...
class MaskedSymbolicNet(SymbolicNet):
    ...
    def set_weights(self,weights):
        # return [self.symbolic_layers[i].W for i in range(self.depth)] + [self.output_weight]
        for i in range(self.sr_unit.depth):
            self.symbolic_layers[i].W = weights[i]
        self.output_weight = weights[self.sr_unit.depth]
